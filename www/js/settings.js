import { dbPromise, bufferedChanges, saveShuffleState, loadShuffleState } from "./database.js"; // NEW: Import new shuffle functions
...
// NEW: Initialize Shuffle Count with daily reset
export async function initShuffleCount(app) {
    ...
    if (!lastShuffleResetDate || lastShuffleResetDate.toDateString() !== today.toDateString()) {
        shouldReset = true; // It's a new day or no reset date recorded
    }

    if (shouldReset) {
        app.shuffleCount = 2; // Reset to 2 at the start of a new day
        await saveShuffleState(app.shuffleCount, today);
    } else {
        app.shuffleCount = shuffleCount; // Use the loaded count for the current day
    }

    // Optional: Update a UI element to display shuffleCount
...
    .then(r => r.json())
    .then(data => {
      app.rssFeedsInput = data.content || ""; // Changed from app.feeds for consistency with component
...
    if (value) { goBackToMainSettings(app); }
  });

  const rssConfigureBtn = document.getElementById('configure-rss-feeds-btn');
  if (rssConfigureBtn) { rssConfigureBtn.addEventListener('click', () => showRssFeeds(app)); }

  const keywordConfigureBtn = document.getElementById('configure-keyword-blacklist-btn');
  if (keywordConfigureBtn) { keywordConfigureBtn.addEventListener('click', () => showKeywordBlacklist(app)); }

  const backButton = document.getElementById('back-button');
  if (backButton) { backButton.addEventListener('click', () => goBackToMainSettings(app)); }
